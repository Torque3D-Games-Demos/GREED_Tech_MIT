...
PhysJointPhysX::PhysJointPhysX(Physics* phys,PhysJointInfo &physJointDescr): PhysJoint(physJointDescr)
{
	...
	physx::PxRigidActor* actor1 = physShape1->getActor();
	physx::PxRigidDynamic* actor2 = physShape2->getActor();
	VectorF jointPos =physJointDescr.pos;//0.5f*(physShape1->getPosition() + physShape2->getPosition());//
	

	physx::PxTransform trt1 = actor1->getGlobalPose();// >getGlobalOrientation();
	/*matrixf
	physx::PxMat33* tr1(trt1.q);
	 */
	physx::PxMat33 tr1 = physx::PxMat33(actor1->getGlobalPose().q);
	...
	*/
	...
	case PhysJointInfo::JT_HINGE:
		{
			physx::PxVec3 axis(1.f,0.f,0.f);
			tr1.transformTranspose(axis);
